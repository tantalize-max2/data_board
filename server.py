...
def get_zone_filter_list(s: dict) -> Optional[List[str]]:
    """战区管理员返回其可管理的所有战区（本角色+额外角色所属战区），
    总经理返回 None（不限战区）"""
    if s.get("is_admin"):
        return None
    zones = [s.get("zone", "")] if s.get("zone") else []
    if zones:
        extra = db.query_all(
            "SELECT DISTINCT zone FROM user_extra_roles WHERE username=%s AND zone!=''",
            (s["username"],))
        for e in extra:
            z = e["zone"]
...
def validate_strong_password(pwd: str) -> Optional[str]:
    """校验强密码：>=7位，含大小写字母+数字+特殊符号。返回 None=通过，否则返回错误描述"""
    if len(pwd) < 7:
        return "密码至少7位"
...
def get_role_rows(s: dict) -> List[dict]:
    ...
    # 战区管理员/指导员：本战区全部数据 + 额外角色所属战区数据
    if s.get("is_zone_admin") or s.get("is_guide"):
        base_zone = s.get("zone", "")
        extra_zones = db.query_all(
            "SELECT DISTINCT zone FROM user_extra_roles WHERE username=%s AND zone!=''",
            (s["username"],))
        all_zones = [base_zone] + [ez["zone"] for ez in extra_zones if ez["zone"] != base_zone]
        placeholders = ",".join(["%s"] * len(all_zones))
        rows = db.query_all(
            f"SELECT * FROM deployment_records WHERE warzone_id IN ({placeholders}) ORDER BY sort_order, id",
            all_zones)
        # 额外战区数据：只保留 combat_role 匹配已赋予角色的
        if extra_zones:
            admin_extra_roles = db.query_all(
                "SELECT role_name FROM user_extra_roles WHERE username=%s",
                (s["username"],))
            extra_role_names = [er["role_name"] for er in admin_extra_roles]
            extra_zone_set = {ez["zone"] for ez in extra_zones}
            result = []
            for r in rows:
                if r.get("warzone_id") == base_zone:
                    result.append(r)
                elif r.get("warzone_id") in extra_zone_set:
                    combat = str(r.get("combat_role", "") or "")
                    if any(match_combat_role(ern, combat) for ern in extra_role_names):
                        result.append(r)
            return [r for r in result if has_substantive(r)]
        return [r for r in rows if has_substantive(r)]
    ...
    # 查询额外角色所属的战区，加入可见战区列表
    extra_zones = db.query_all(
        "SELECT DISTINCT zone FROM user_extra_roles WHERE username=%s AND zone!=''",
        (s["username"],))
    all_zones = [user_zone] + [ez["zone"] for ez in extra_zones if ez["zone"] != user_zone]
    placeholders = ",".join(["%s"] * len(all_zones))
...
def overview(request: Request):
    ...
    if not s.get("is_admin"):
        ...
        if me:
            ...
            ez = db.query_all(
                "SELECT DISTINCT zone FROM user_extra_roles WHERE username=%s AND zone!=''",
                (s["username"],))
            extra_user_zones = [e["zone"] for e in ez if e["zone"] != user_zone]
    # 查询兵种角色（过滤空 role_name）
    zone_roles_map: Dict[str, List[str]] = {}
    query_zones = [user_zone] + extra_user_zones if user_zone else None
    if query_zones:
        for z in query_zones:
            for u in db.query_all("SELECT role_name FROM users WHERE is_active=1 AND is_admin=0 AND zone=%s AND role_name!=''", (z,)):
                zone_roles_map.setdefault(z, []).append(u["role_name"])
    else:
    ...
    # 把用户已赋予的额外角色也加入所属战区的兵种列表
    if user_zone:
        extra_roles = db.query_all(
            "SELECT role_name FROM user_extra_roles WHERE username=%s AND role_name!=''",
            (s["username"],))
        for er in extra_roles:
            if er["role_name"] not in zone_roles_map.get(user_zone, []):
                zone_roles_map.setdefault(user_zone, []).append(er["role_name"])
    # 普通用户返回本战区+额外战区，管理员返回全部
    visible_zones = [user_zone] + extra_user_zones if user_zone else list(zcount.keys())
...
def admin_users(request: Request, q: str = "", zone: str = ""):
    """人员列表（支持关键词 q 与战区过滤）"""
    s = require_zone_admin(request)
    zfl = get_zone_filter_list(s)
    admin_zone = s.get("zone", "")
    admin_extras = db.query_all(
        "SELECT role_name FROM user_extra_roles WHERE username=%s",
        (s["username"],))
    admin_extra_roles = [ae["role_name"] for ae in admin_extras]
    ...
    if zfl:
        ...
        for z in zfl:
            if z == admin_zone:
                clauses.append("zone=%s"); args.append(z)
            else:
                # 额外战区：只显示管理员被赋予的角色
                if admin_extra_roles:
                    ph = ",".join(["%s"] * len(admin_extra_roles))
                    clauses.append(f"(zone=%s AND role_name IN ({ph}))")
                    args.append(z)
                    args.extend(admin_extra_roles)
        if clauses:
    ...
    if q:
        ...
        kw = f"%{q}%"
        args += [kw, kw, kw, kw]
    if zone and not zfl and zone != 'all':
        sql += " AND zone=%s"
        args.append(zone)
    sql += " ORDER BY is_admin DESC, is_zone_admin DESC, zone, role_name"
    rows = db.query_all(sql, args)
    return {"total": len(rows), "users": rows, "me": get_user_info(s["username"]), "is_zone_admin": bool(zfl)}
...
def admin_get_access(request: Request, role_id: str = "", q: str = ""):
    ...
    # 获取所有唯一角色名（按战区分组）
    if zfl:
        placeholders = ",".join(["%s"] * len(zfl))
        raw_roles = db.query_all(
            f"SELECT DISTINCT role_name, zone, zone_name FROM users WHERE is_admin=0 AND is_zone_admin=0 AND is_active=1 "
            f"AND zone IN ({placeholders}) AND role_name!='' ORDER BY zone_name, role_name",
            zfl)
    else:
        raw_roles = db.query_all(
    ...
    return {
        ...
        "zone_filter": zfl[0] if zfl and len(zfl)==1 else (zfl or None),
    }
...
async def admin_set_access(request: Request):
    ...
    if zfl:
        placeholders = ",".join(["%s"] * len(zfl))
        exists = db.query_one(
            f"SELECT id FROM users WHERE role_name=%s AND zone IN ({placeholders}) AND is_active=1 LIMIT 1",
            [role_id] + zfl)
        if not exists:
...
def admin_get_user_roles(uid: int, request: Request):
    ...
    extras = db.query_all("SELECT role_name, zone FROM user_extra_roles WHERE username=%s ORDER BY role_name",
                          (user["username"],))
    # 所有可选角色名（按战区分组，额外战区只显示管理员被赋予的角色）
    zfl = get_zone_filter_list(s)
    if zfl:
        placeholders = ",".join(["%s"] * len(zfl))
        raw = db.query_all(
            f"SELECT DISTINCT role_name, zone, zone_name FROM users "
            f"WHERE is_active=1 AND role_name!='' AND zone IN ({placeholders}) ORDER BY zone_name, role_name",
            zfl)
    else:
        raw = db.query_all(
    ...
    admin_zone = s.get("zone", "")
    admin_extras = db.query_all(
        "SELECT role_name, zone FROM user_extra_roles WHERE username=%s",
        (s["username"],))
    admin_extras_set = {(ae["zone"], ae["role_name"]) for ae in admin_extras}
    zones_map = {}
    for r in raw:
        z = r["zone"]; rn = r["role_name"]
        if not zfl:
            # 全球管理员：显示所有战区角色
            zones_map.setdefault(z, {"zone": z, "zone_name": r["zone_name"], "roles": []})
            if rn not in zones_map[z]["roles"]:
                zones_map[z]["roles"].append(rn)
        elif z == admin_zone:
            zones_map.setdefault(z, {"zone": z, "zone_name": r["zone_name"], "roles": []})
            if rn not in zones_map[z]["roles"]:
                zones_map[z]["roles"].append(rn)
        elif (z, rn) in admin_extras_set:
            zones_map.setdefault(z, {"zone": z, "zone_name": r["zone_name"], "roles": []})
            if rn not in zones_map[z]["roles"]:
                zones_map[z]["roles"].append(rn)
    ...
    return {
        ...
        "extra_roles": [{"role_name": r["role_name"], "zone": r.get("zone", "")} for r in extras],
...
async def admin_set_user_roles(uid: int, request: Request):
    ...
    if zf and user["zone"] != zf:
        raise HTTPException(403, "只能管理本战区人员")
    body = await request.json()
    roles = body.get("roles", [])  # [{role_name, zone}, ...]
    username = user["username"]
    db.execute("DELETE FROM user_extra_roles WHERE username=%s", (username,))
    if roles:
        # 验证：本战区所有角色有效；额外战区只允许管理员被赋予的角色
        zfl = get_zone_filter_list(s)
        valid_pairs = set()
        rows = db.query_all("SELECT DISTINCT role_name, zone FROM users WHERE is_active=1 AND role_name!=''")
        for vr in rows:
            z = vr["zone"]; rn = vr["role_name"]
            if not zfl:
                # 全球管理员：所有战区角色都有效
                valid_pairs.add((z, rn))
            elif z == s.get("zone", ""):
                valid_pairs.add((z, rn))
            elif z in zfl:
                admin_has = db.query_one(
                    "SELECT id FROM user_extra_roles WHERE username=%s AND role_name=%s AND zone=%s",
                    (s["username"], rn, z))
                if admin_has:
                    valid_pairs.add((z, rn))
        clean = []
        for item in roles:
            rn = item.get("role_name", "").strip()
            z = item.get("zone", "").strip()
            if rn and rn != user["role_name"] and (z, rn) in valid_pairs:
                clean.append((username, rn, z))
        if clean:
            db.executemany("INSERT IGNORE INTO user_extra_roles(username,role_name,zone) VALUES(%s,%s,%s)",
                           clean)
...
def admin_records(request: Request, battle_id: str = "", zone_id: str = ""):
    ...
    if zfl:
        placeholders = ",".join(["%s"] * len(zfl))
        sql += f" AND warzone_id IN ({placeholders})"; args.extend(zfl)
    if battle_id:
        sql += " AND battle_id=%s"; args.append(battle_id)
    if zone_id and not zfl:
...
async def admin_delete_record(rid: int, request: Request):
    ...
    if not rec:
        raise HTTPException(404, "记录不存在")
    if zfl and rec["warzone_id"] not in zfl:
        raise HTTPException(403, "只能管理本战区的记录")
